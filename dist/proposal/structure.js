...
export function buildProposalStructure(language, original, smartpen) {
  ...
  const learningBody = (source, prefix) => {
    ...
    const body = prefix === 'smartpen-' ? smartpenWritingDemo(language) + item.body + smartpenFlow(language) : item.body;
  ...
  const overviewHTML = `
    ...
    <section class="feature-detail">
      ...
      ${paragraph(copy('系統預計包括 iOS 及 Android 原生應用程式，以及供電腦使用的網頁介面。家長以手機為主，老師可使用平板或電腦，中心職員及總部以電腦處理日常工作。學生可選擇兩種作答方式：方案一以直向平板作答，方案二保留紙本工作紙及資料夾，以墨水智能筆在專用點紋工作紙上書寫，連線時逐筆同步。', 'The proposed system includes native iOS and Android applications and a web interface for computers. Parents mainly use phones, teachers use tablets or computers, and centre staff and HQ use computers for everyday work. Students can choose between two writing methods: portrait tablets in Solution 1, or familiar binders and specially patterned worksheets with ink smartpens streaming strokes while connected in Solution 2.'))}
      ${paragraph(copy('<strong>學生及老師：</strong>派發習作、作答、批改、改正、學習進度及集印鼓勵。學生的書寫方式見第 2 節；教材準備、派發及老師跟進流程見第 3 節。', '<strong>Students and teachers:</strong> assignments, answers, marking, corrections, learning progress and stamp rewards. Section 2 describes how students write; section 3 covers material preparation, assignment and teacher follow-up.'))}
    ...
    <div class="document-links"><a href="#learning">${copy('2. 學生體驗', '2. Student experience')}</a><a href="#materials">${copy('3. 教材', '3. Teaching materials')}</a><a href="#system">${copy('4. 中心及總部管理', '4. Centre & HQ management')}</a><a href="#parent">${copy('5. 家長應用程式', '5. Parent app')}</a></div>`;

  const learning = {
    id: 'learning',
    title: copy('學生體驗', 'Student experience'),
    heading: copy('學生體驗', 'Student experience'),
    intro: copy('學生可在平板上以觸控筆作答，或保留紙本工作紙，以智能筆書寫並同步筆跡。以下比較兩種書寫及使用體驗。', 'Students can write on a tablet with a stylus, or keep paper worksheets and use an ink smartpen to capture their handwriting. Compare the two writing experiences below.'),
    body: `<section class="learning-comparison" id="learning-comparison" aria-labelledby="learning-comparison-title">
      <header class="learning-comparison-heading"><h3 id="learning-comparison-title">${copy('選擇作答方式', 'Choose how students write')}</h3><p>${copy('切換下方選項，查看平板或智能筆的學生體驗。', 'Switch between the options to explore the tablet or smartpen student experience.')}</p></header>
      <nav class="solution-switch" id="solution-switch" aria-label="${copy('選擇作答方式', 'Choose how students write')}"><a data-solution="1" aria-controls="learning-solution-1" href="?solution=1#learning"><span class="solution-number">${copy('方案一', 'Solution 1')}</span><strong>${copy('平板無紙化', 'Paperless tablets')}</strong><span class="solution-description">${copy('在平板上直接書寫及作答', 'Write and answer directly on a tablet')}</span></a><a data-solution="2" aria-controls="learning-solution-2" href="?solution=2#learning"><span class="solution-number">${copy('方案二', 'Solution 2')}</span><strong>${copy('紙本＋智能筆', 'Paper + smartpen')}</strong><span class="solution-description">${copy('保留紙本書寫，筆跡自動同步至系統', 'Keep writing on paper, with automatic handwriting sync')}</span></a></nav>
  ...
  const materials = {
    ...
    intro: copy('從現有教材數碼化、新教材編製，到課堂派發、批改及學習跟進，老師可在同一系統完成教學工作，並運用學校資訊及 AI 準備合適的練習。', 'From digitising existing materials and creating new content to assigning worksheets, marking and following up learning, teachers can work in one system and use school information and AI to prepare suitable practice.'),
    body: subsection(chapter(original, 'library'), 'library') +
      subsection(chapter(original, 'teacher'), 'teacher') +
