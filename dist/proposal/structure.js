...
export function buildProposalStructure(language, original, smartpen) {
  ...
  const overviewHTML = `
    <header class="document-header">
      ...
      <p class="document-summary">${copy('為 MathConcept 建立涵蓋學生學習、教材管理、中心營運及家長服務的系統。教學部分提供「平板無紙化」及「紙本＋智能筆」兩種選擇，供中心比較；中心營運及家長功能兩方案相同，在本建議書只列一次。', 'A system for MathConcept covering student learning, teaching materials, centre operations and parent services. The teaching section offers two approaches to compare: paperless tablet learning or paper with smartpens. Centre operations and parent functions are the same for both approaches and appear only once in this proposal.')}</p>
    ...
    <section class="feature-detail">
      ...
      ${paragraph(copy('系統擬包括 iOS 及 Android 原生應用程式，以及供電腦使用的網頁介面。家長以手機為主，老師可使用平板或電腦，中心職員及總部以電腦處理日常工作。學生的作答方式按所選教學方案安排：方案一以直向平板作答，方案二保留紙本工作紙及資料夾，以智能筆記錄筆跡。支援裝置及完整使用流程須在試行時確認。', 'The proposed system includes native iOS and Android applications and a web interface for computers. Parents mainly use phones, teachers use tablets or computers, and centre staff and HQ use computers for everyday work. Student writing depends on the chosen teaching approach: portrait tablets in Solution 1, or familiar paper worksheets and binders with smartpen handwriting capture in Solution 2. Supported devices and the complete workflow will be validated during a pilot.'))}
  ...
  const learning = {
    ...
    intro: copy('在這裏切換學生的學習方式，查看相應的學生體驗、老師流程及教材準備。選擇只影響本節；第 3 節的中心及總部管理、第 4 節的家長應用程式均為兩方案共用。', 'Switch the learning approach here to compare the student experience, teacher workflow and material preparation. The selection affects only this section. Centre and HQ management in section 3 and the parent app in section 4 are shared by both approaches.'),
    body: `<nav class="solution-switch" id="solution-switch" aria-label="${copy('選擇教學方案', 'Choose a teaching approach')}"><a data-solution="1" href="?solution=1#learning">${copy('方案一：平板無紙化', 'Solution 1: paperless tablets')}</a><a data-solution="2" href="?solution=2#learning">${copy('方案二：紙本＋智能筆', 'Solution 2: paper + smartpen')}</a></nav>
      ...
      <section id="learning-solution-2" class="learning-option" data-learning-solution="2" aria-labelledby="learning-solution-2-title"><h3 id="learning-solution-2-title">${copy('方案二：保留紙本，以智能筆建立數碼紀錄', 'Solution 2: retain paper and capture learning records with smartpens')}</h3><p class="solution-demo-note">${copy('智能筆配對、工作紙識別及筆跡同步屬建議功能；本網站未連接實體智能筆。以下說明擬議流程，須配合實際教材及設備試行驗證。', 'Pen assignment, worksheet identification and handwriting synchronisation are proposed functions. This website does not connect to a physical smartpen. The workflow below requires validation with real materials and equipment during a pilot.')}</p>${learningBody(smartpen, 'smartpen-')}</section>
  ...
  const chapters = [
    ...
    learning,
    {
      ...
      intro: copy('本節功能由兩個教學方案共用。報名、排課、請假補堂、收費及跨中心管理採用相同流程，以下只列出一次。', 'These functions are shared by both teaching approaches. Enrolment, scheduling, leave and make-up lessons, billing and multi-centre management follow the same workflows and are described once below.'),
