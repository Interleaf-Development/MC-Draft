...
export function getSmartpenProposal(language, original) {
  ...
  const chapters = [
    { id: 'vision', title: copy('方案二概覽', 'Solution 2 overview') },
    {
      ...
      intro: copy('學生拿起的是一支真正有墨水的筆，在保留原有版面及書寫空間的工作紙上作答。設備連線正常時，筆尖寫下的數字、算式和圖形會逐筆出現在系統；不需要完成整頁後再拍照或掃描。學生繼續面對紙張，老師則多了一份可即時查看、重播及跟進的學習紀錄。', 'Students use a real ink pen on worksheets that retain their familiar layout and working space. With a working connection, numbers, equations and diagrams appear in the system stroke by stroke as they are written, without photographing or scanning the completed page. Students keep their attention on paper, while teachers gain a record they can view, replay and follow up.'),
      body:
        featureGroup('專用紙、墨水筆與識別演算法，記下每一筆', 'Special paper, an ink pen and decoding algorithms capture each stroke', [
          ['<strong>紙張：</strong>工作紙印有細微的識別點紋，外觀看起來接近一般工作紙，在正常閱讀距離並不顯眼，題目、圖形及作答空間均可保留。特別之處在於印刷的點紋，紙張本身不需要螢幕或電池；不能直接以沒有相容點紋的普通影印紙代替。', '<strong>The paper:</strong> worksheets carry a fine identification pattern that is unobtrusive at normal reading distance, while retaining the questions, diagrams and working space. The special element is the printed pattern; the paper needs no screen or battery. Ordinary copies without the compatible pattern cannot replace it.'],
          ['<strong>智能筆：</strong>筆芯會在紙上留下真實墨水，學生照常握筆、寫字及列式。筆尖附近的感應器在書寫時讀取紙上的點紋，持續記錄筆尖位置及書寫時間。數碼紀錄隨下筆產生，並非靠事後辨識一張完成的工作紙照片。', '<strong>The pen:</strong> its refill leaves real ink on paper as students write and work through calculations. A sensor near the tip reads the printed pattern during writing, capturing the tip’s position and timing. The digital record is produced as the pen moves, rather than extracted later from a photograph of the finished worksheet.'],
          ['<strong>識別及同步演算法：</strong>把點紋所代表的頁面與位置，連同每筆的時間和先後次序，還原成數碼筆跡，再按已設定的配對歸入學生及習作。記錄筆跡與理解數學答案是兩個步驟；AI 辨識、批改及學習分析會建立在這份原始紀錄之上，保留原筆跡供老師核對。', '<strong>Decoding and synchronisation:</strong> page and position information, timing and stroke order are assembled into digital handwriting, then linked to the assigned pupil and worksheet. Capturing handwriting and understanding a mathematical answer are separate steps. AI recognition, marking and learning analysis build on the original record, which remains available for teachers to check.']
        ]) +
        featureGroup('保留資料夾習慣，連線時即寫即存', 'Keep familiar binders, with connected writing captured as it happens', [
          ['老師仍從學習進度表選取已核准工作紙，指定學生及堂課或功課用途，再安排列印及派發。上課前配對學生、筆及工作紙，記下頁次、教材版本及該次派發；補印、收回、換筆或重新派發亦保留紀錄，避免不同學生或不同次作答混在一起。中心可選擇個人領用或課堂借用。', 'Teachers select approved worksheets from the progress chart, assign them as classwork or homework, and arrange printing and distribution. Before class, the pupil, pen and worksheet are associated with the page, material version and assignment. Reprints, withdrawals, replacement pens and reassignment retain their own records so different pupils or attempts do not become mixed. Centres can allocate pens individually or lend them during lessons.'],
          ['連線正常時，筆跡隨書寫同步至相容裝置，再由系統提供給獲授權老師查看。支援離線儲存的筆型可在未連線時暫存筆跡，重新連線後補傳；離線期間，老師畫面不會即時更新。系統會分清已收到、待同步及需要處理的紀錄，課後再核對完整性，避免把「未收到筆跡」誤作「學生未作答」。家中功課亦按相同原則處理，實際同步速度、容量及同時使用人數會在試行時確認。', 'With a working connection, handwriting streams to a compatible device and the system makes it available to authorised teachers. Pens that support offline storage can retain writing for later transfer, but the teacher’s view does not update live while disconnected. Received, pending and exception records are distinguished and checked after class, so missing data is not mistaken for unanswered work. Homework follows the same principle. Sync performance, storage capacity and simultaneous users will be tested during the pilot.'],
          ['資料夾保留「已完成、目前、日後」分類。學生照常作答、交功課及改正，已完成習作保留重溫；日後習作由老師保管並決定何時派發及在帳戶內開放。系統不會向學生或家長顯示未開放教材，但已派出的紙張不能鎖上，仍須由老師控制實際派發時間。連線中斷時可先以紙本繼續教學，再按復原程序補回紀錄。', 'Binders retain past, current and future sections. Students answer, hand in homework and make corrections as usual, keeping completed work for revision. Teachers hold future work and decide when to distribute it and release it digitally. Unreleased material remains unavailable to pupil and parent accounts; teachers still control physical distribution because a sheet already handed out cannot be locked. Paper remains available for teaching during interruptions while records are recovered.']
        ]) +
    ...
    },
    {
      ...
      intro: copy('我們希望老師既能保留面對面的教學方式，也能看見學生寫下答案的過程。連線時可即時查看作答；課後可逐筆重播、回看停頓位置及跟進改正。以下分析及教學功能是建議開發方向，會按實際筆跡資料、題型及課堂試行結果逐步確認。', 'We want teachers to keep face-to-face teaching while gaining a view of how students arrive at their answers. Connected work can be viewed as it happens, then replayed stroke by stroke to revisit pauses and corrections. The following analysis and teaching features are proposed development directions, to be validated against real handwriting, question types and classroom trials.'),
      body:
        featureGroup('從逐筆紀錄，看見學習節奏與需要協助的地方', 'Use stroke records to explore learning patterns and support needs', [
          ['<strong>即時跟進及作答重播：</strong>老師可在同一畫面查看已連線學生正在寫的步驟，選取其中一份放大跟進，或課後按時間重播原作答，回看先列了甚麼式、在哪一步停筆、何時再寫或重寫。配合已設定的題目範圍，亦可從某一題直接開始重播；這可延伸至已連線的家中功課及遙距講解。', '<strong>Live follow-up and replay:</strong> teachers could view connected students’ current steps together, open one worksheet for closer attention, or replay the original answer later to revisit the order of working, pauses and rewritten steps. With question regions defined, replay could start at a particular question. The same approach could support connected homework and remote explanations.'],
          ['<strong>書寫用時與停頓線索：</strong>利用落筆、抬筆、位置及時間紀錄，可估算一筆寫了多久、兩筆之間停了多久，以及一題內實際書寫與間隔時間的分佈；支援的筆型亦可提供力度變化。譬如學生列完算式後停了數秒，才寫出答案，老師便可回到該位置了解原因。停筆可能是思考、閱讀、聽老師說話或等待，不能單憑時間判斷學生不懂；分析亦須先排除缺漏或未同步的紀錄；未開始書寫前的閱讀時間，不能單靠筆跡得知。', '<strong>Writing time and pauses:</strong> pen-down, pen-up, position and time records allow estimates of stroke duration, gaps between strokes, and the distribution of writing and gaps within a question to be calculated. Supported pens can also provide pressure changes. If a student pauses after setting out an equation, the teacher could revisit that point and ask why. A pause may mean thinking, reading, listening or waiting; timing alone cannot establish confusion. Analysis must first account for missing or unsynchronised data. Pen records alone cannot reveal reading time before the first stroke.'],
          ['<strong>個人化跟進與教學改進：</strong>把同一學生在相近題型的作答次序、正確率、重寫及用時一併比較，可讓老師了解練習後是否更熟練，並由 AI 草擬針對性的提示、練習及課後摘要，再由老師選用。以去識別化資料整理全班常見的停頓題目或錯誤步驟，亦可協助調整教材、講解及練習次序。家長看到的是老師確認的進展及建議；速度不是唯一標準，也不會憑停頓推斷情緒、專注力或能力。', '<strong>Personal follow-up and teaching improvement:</strong> comparing one student’s order of working, accuracy, rewritten steps and timing on similar questions could help teachers review fluency over time. AI could draft targeted hints, exercises and summaries for teacher selection. De-identified class patterns could highlight questions with frequent pauses or errors and inform materials, explanations and practice order. Parents receive teacher-confirmed progress and advice. Speed is not the sole standard, and pauses do not establish emotion, attention or ability.']
        ]) +
    ...
    },
    {
      ...
      body:
        featureGroup('整理現有教材，驗證工作紙印製', 'Organise existing materials and validate printing', [
          ['沿用年級、課題、教材系列及工作紙編號建立目錄，核對檔案完整性、重複內容及使用版本。已核准工作紙保留原有算式、圖形及作答空間；印製時加入相容的細微識別點紋，讓筆讀取頁面及位置，再由系統連結學生、教材版本及派發紀錄。會先用實物確認點紋的觀感、紙張及墨水書寫效果；普通影印或任意縮放不能直接假設仍可辨識。', 'Build the catalogue around existing grades, topics, series and worksheet numbers, checking completeness, duplicates and versions. Approved sheets retain their equations, diagrams and working space. A compatible fine pattern allows the pen to identify the page and position, which the system links to the pupil, material version and assignment. Physical samples will be checked for pattern visibility, paper feel and ink writing; ordinary photocopies or arbitrary scaling cannot be assumed to remain readable by the pen.'],
