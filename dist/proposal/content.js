...
detail('Timetable operations',`<p>Drag a lesson to another time or into the leave bin, or drag a student awaiting a make-up lesson back onto the timetable. Keyboard controls are also available. Before confirmation, check teacher availability, class capacity and time clashes.</p>`)+
  ...
  detail('Changes to lesson counts',`<p>Before a regular timetable change, show the original and proposed lesson dates and compare their counts. For x → x+1 lessons, admin staff must approve the extra lesson at the original fee or exclude it. For x → x−1 lessons, they must choose whether to grant a corresponding make-up credit.</p>`)+
  ...
  detail('Change records',`<p>An approved schedule change updates the actual lesson dates and creates an amended receipt. The amendment retains the original payment date, receipt date and paid amount, and separately records its actual amendment date and reason. The original and all previous receipt versions remain available.</p>`))+
 feature('Invoice review and bank reconciliation', 'Billing follows the sequence: invoice → parent payment proof → staff review and approval → receipt. Bank statements are then checked to confirm whether actual credits match the receipt records. Receipt issuance and bank reconciliation have separate progress records, helping staff follow up unfinished work. Use AI to help review payment proof, issue receipts and complete final reconciliation.',
  ...
  detail('Invoice creation and payment deadlines',table(['Charge','When the invoice is issued','Payment or enrolment rule'],[
   ...
   ['Recurring tuition','HK$2,000 buys a nominal eight-lesson package valid for two calendar months. Students can have different periods, such as September–October or October–November. The usual seven, eight or nine weekly lessons falling within a period do not change the fee or create a refund; the teaching schedule averages 48 weeks annually.','Automatically issue the invoice on the 20th of the month before the next period, due on the following month’s 20th. For example, October–November is invoiced on 20 September and due on 20 October. Renewal payment approval does not activate enrolment again.'],
