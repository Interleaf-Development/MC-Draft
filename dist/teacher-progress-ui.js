...
export function createTeacherProgressUI({ getState, getTutorId, change, render, onStudentChange, openAssignment, openFolder, openStudentView }) {
  ...
  function classNavigation() {
    ...
    }).join('')+'</div>'+action('next-class', arrow('next'), 'aria-label="Next class"'+(index < 0 || index === list.length-1 ? ' disabled' : ''), 'teacher-progress-class-arrow')+'</div><div class="teacher-progress-students-list" role="group" aria-label="Students in selected class">'+(session ? session.students.map(student => action('student', '<span>'+esc(student.name)+'</span><small>'+esc(student.level)+'</small>', 'data-student="'+esc(student.id)+'" aria-pressed="'+(selectedStudent === student.id)+'"', 'teacher-progress-student'+(selectedStudent === student.id ? ' is-active' : ''))).join('') : '<p class="teacher-progress-empty">No classes scheduled for this teacher.</p>')+'</div></section>';
  }
