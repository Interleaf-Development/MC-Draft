...
function render() {
  ...
  $('#app').innerHTML = '<div class="app-shell"><aside class="sidebar"><div class="wordmark">Math<span>Concept</span></div><div class="centre-label">('+t(centre.branch)+')</div><div class="nav-section">' + t({admin:'Centre',teacher:'Teaching',parent:'Family',student:'My classroom'}[ui.role]) + '</div><nav class="nav-list" aria-label="'+t('Main navigation','主要導覽')+'">' + nav.map(([id, ic, label]) => action('navigate', icon(ic) + '<span>' + t(label) + '</span>', 'nav-item' + (ui.page === id ? ' active' : ''), 'data-page="' + id + '"' + (ui.page === id ? ' aria-current="page"' : ''))).join('') + '</nav><div class="sidebar-bottom"><div class="flex">' + avatar(user) + '<div><div class="small strong">' + esc(user.name) + '</div><div class="user-caption">' + esc(t(user.title || user.level)) + '</div></div></div></div></aside><header class="topbar"><div class="mobile-brand">' + action('toggle-menu', icon('menu'), 'icon-btn mobile-menu', 'aria-label="'+t('Open navigation','開啟導覽')+'"') + '<span class="brand-lockup"><span class="wordmark">Math<span>Concept</span></span><span class="brand-branch">('+t(centre.branch)+')</span></span></div><div class="role-switch" role="group" aria-label="'+t('Demo role','示範角色')+'">' + ['admin','teacher','parent','student'].map(r => action('role', t(r[0].toUpperCase() + r.slice(1)), ui.role === r ? 'active' : '', 'data-role="' + r + '" aria-pressed="' + (ui.role === r) + '"')).join('') + '</div><div class="topbar-actions"><span class="date">'+t('Wednesday, 30 September 2026','2026 年 9 月 30 日（星期三）')+'</span>' + action(ui.role==='parent'?'demo-controls':'demo-info', 'Demo', 'demo-label', 'aria-label="'+t(ui.role==='parent'?'Demo controls':'About this demo')+'"') + action('reset-demo', icon('reset') + '<span class="reset-label">'+t('Reset')+'</span>', 'btn ghost small', 'aria-label="'+t('Reset demo')+'"') + '</div></header><main class="main ' + (['parent','student'].includes(ui.role) ? 'family-main' : '') + '" id="main-content">' + page() + '</main><nav class="mobile-bottom-nav" aria-label="'+t('Mobile navigation','手機導覽')+'">' + nav.slice(0,5).map(([id,ic,label]) => action('navigate', icon(ic) + '<span>' + t(label === 'Billing & reconciliation' ? 'Billing' : ({'My classroom':'Classroom','My work':'My work','My schedule':'Schedule'}[label]||label)) + '</span>', ui.page === id ? 'active' : '', 'data-page="' + id + '"')).join('') + '</nav></div>';
  attachDrawing();
...
function parentOverview(){
 ...
 return heading('家長主頁',childSwitch())+'<section class="family-banner"><div class="banner-copy"><p class="eyebrow">下一課 · '+s.name+'</p><h2>'+(next?dateLabel(next.date,{weekday:'long'}):'等候安排課堂')+'</h2><p>'+(next?time(next.start)+'–'+time(next.start+next.duration)+' · '+tutorName(next.tutor):'中心會與你確認上課時間。')+'</p><div class="mt-16">'+action('show-checkin',icon('qr')+' 出席二維碼','btn primary')+'</div></div>'+art(1,'','MathConcept 朋友')+'</section><div class="family-grid"><section><div class="between mb-16"><h2>課堂近況</h2>'+action('navigate','查看全部','inline-link','data-page="handbook"')+'</div>'+(notes.length?noteCard(notes[0]):'<div class="panel">'+empty('學習手冊已準備好','課堂紀錄將顯示於這裏。',15)+'</div>')+'</section><aside class="stack"><section class="panel"><div class="panel-head"><h3>學費</h3>'+tag(invoice?'繳費限期 '+dateLabel(invoice.due):'Receipt available',invoice?'amber':'green')+'</div><div class="panel-body"><p class="small muted">'+esc(content(displayInvoice?.period)||'暫無繳費通知')+'</p><h2 class="mt-8">'+money(displayInvoice?.amount||0)+'</h2><p class="small muted mt-8">'+esc(content(displayInvoice?.description)||'中心會與你確認學費。')+'</p><div class="mt-16">'+action('navigate',invoice?'View invoice':'View receipt','btn w-full','data-page="payments"')+'</div></div></section><section class="panel"><div class="panel-body"><div class="flex">'+icon('message')+'<h3>聯絡中心</h3></div><p class="small muted mt-8">對 '+s.name.split(' ')[0]+' 的課堂有疑問？</p><div class="mt-16">'+action('navigate','傳送訊息 '+icon('arrow','sm'),'inline-link','data-page="messages"')+'</div></div></section></aside></div>';
}
...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
  ...
  const i=state.invoices.find(i=>i.id===id);modal(t('Invoice '+id,'繳費通知 '+id),'<div class="receipt-paper"><div class="wordmark">Math<span>Concept</span></div><p class="small muted mt-16">'+t(centre.name)+'</p><dl class="detail-grid"><div><dt>' + t('Student','學生') + '</dt><dd>'+studentById(i.studentId).name+'</dd></div><div><dt>' + t('Tuition period','學費期數') + '</dt><dd>'+content(i.period)+'</dd></div><div><dt>' + t('Issued','發出日期') + '</dt><dd>'+dateLabel(i.issued)+'</dd></div><div><dt>' + t('Payment due','繳費限期') + '</dt><dd>'+dateLabel(i.due)+'</dd></div></dl><p class="small">'+esc(content(i.description))+'</p><div class="receipt-total"><span>' + t('Total','總額') + '</span><span>'+money(i.amount)+'</span></div></div>',action('close-modal','Close','btn')+(i.proof?action('view-proof','Payment proof','btn','data-id="'+id+'"'):'')+(i.receiptId?action('view-receipt','Receipt','btn primary','data-id="'+i.receiptId+'"'):action('submit-proof',i.proof?'Replace proof':'Add payment proof','btn primary','data-id="'+id+'"')));
