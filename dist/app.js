...
function invoiceStatus(invoice){
 ...
 return invoice.proof?'proof':'awaiting';
}
function billingPage(){
 const key='invoices',c=collection(key);
 const shell=heading('Billing & reconciliation',action('export-report',icon('download')+' Export report','btn'))+tabs(['Invoices','Reconciliation','HQ report'],ui.billingTab,'billing-tab');
 if(ui.billingTab==='Reconciliation')return shell+bankCheckUI.render();
 if(ui.billingTab==='HQ report')return shell+directorReview();
 const labels={issued:'Receipt issued',review:'Proof needs review',proof:'Needs screening',awaiting:'Awaiting proof'};
 const list=state.invoices.filter(item=>!c.query||matchesStudent(item.studentId,c.query)||[item.id,item.proofReference].join(' ').toLowerCase().includes(c.query.toLowerCase())).filter(item=>c.status==='all'||invoiceStatus(item)===c.status);
 const p=collectionPage(key,list);
 const filters='<div class="filters">'+searchControl(key,'Search invoices','Student, ID, parent or reference')+filterControl(key,'status','Invoice status',[['all','All invoices'],['review','Proof needs review'],['proof','Needs screening'],['awaiting','Awaiting proof'],['issued','Receipt issued']])+action('clear-list','Clear filters','btn ghost small','data-list="'+key+'"')+'</div>';
 const rows=p.items.map(i=>{const student=studentById(i.studentId),status=invoiceStatus(i);return '<tr><td><div class="flex">'+avatar(student,'small')+'<div class="row-title">'+student.name+'<div class="row-meta">'+student.number+' · '+i.id+'</div></div></div></td><td><div>'+i.period+'</div><div class="row-meta">'+esc(i.description)+'</div></td><td class="strong nowrap">'+money(i.amount)+'</td><td class="nowrap">'+dateLabel(i.due)+'</td><td>'+tag(labels[status],status==='issued'?'green':status==='review'?'amber':'')+'</td><td class="table-actions">'+(i.receiptId?action('view-receipt','View receipt','btn small','data-id="'+i.receiptId+'"')+action('view-proof','Proof','btn ghost small','data-id="'+i.id+'"'):i.proof?action('view-proof','Review proof','btn small','data-id="'+i.id+'"'):action('submit-proof','Add proof','btn small','data-id="'+i.id+'"'))+'</td></tr>';}).join('');
 return shell+filters+'<section class="panel"><div class="table-scroll"><table><thead><tr><th>Student / invoice</th><th>Tuition period</th><th>Amount</th><th>Due</th><th>Status</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div>'+(p.total?'':empty('No matching invoices','Try another search or filter.'))+pager(key,p)+'</section>';
}
...
function parentPayments(){
 ...
 return heading('Payments',childSwitch())+'<div class="stack">'+(invoices.length?invoices.map(i=>{const status=invoiceStatus(i);return '<section class="panel"><div class="panel-head"><div><h3>'+i.period+'</h3><p class="small muted mt-8">'+i.id+' · Issued '+dateLabel(i.issued)+'</p></div>'+tag(i.receiptId?'Receipt issued':status==='review'?'Proof needs attention':i.proof?'Proof submitted':'Due '+dateLabel(i.due),i.receiptId?'green':status==='review'?'amber':i.proof?'blue':'amber')+'</div><div class="panel-body between wrap"><div><h2>'+money(i.amount)+'</h2><p class="small muted mt-8">'+esc(i.description)+'</p></div><div class="flex">'+action('view-invoice','View invoice','btn','data-id="'+i.id+'"')+(i.receiptId?action('view-receipt',icon('receipt')+' Receipt','btn primary','data-id="'+i.receiptId+'"'):i.proof?action('view-proof',status==='review'?'View result':'View proof','btn','data-id="'+i.id+'"'):action('submit-proof',icon('upload')+' Submit payment proof','btn primary','data-id="'+i.id+'"'))+'</div></div></section>';}).join(''):'<div class="panel">'+empty('No invoices yet','Your centre will issue an invoice after enrolment.',14)+'</div>')+'</div>';
}
...
function openProof(invoiceId){
 ...
 if(note)ui.matchDraft={receiptId:ui.matchReceiptId,bankId:ui.selectedBank,note:note.value};
...
function receiptDialog(receiptId){
 ...
 modal('Receipt '+r.id,'<div class="receipt-paper"><div class="between"><div class="wordmark">Math<span>Concept</span></div><span class="eyebrow">Receipt</span></div><p class="small muted mt-16">'+centre.name+'</p><dl class="detail-grid"><div><dt>Receipt no.</dt><dd>'+r.id+'</dd></div><div><dt>Issued</dt><dd>'+dateLabel(r.issuedDate)+'</dd></div><div><dt>Parent / guardian</dt><dd>'+studentById(r.studentId).parent+'</dd></div><div><dt>Student</dt><dd>'+studentById(r.studentId).name+'</dd></div></dl><p class="strong small">'+esc(i?.description||'Regular programme')+'</p><p class="small muted mt-8">'+esc(i?.period||'')+' · '+r.invoiceId+'</p><div class="receipt-total"><span>Payment amount</span><span>'+money(r.amount)+'</span></div><p class="small muted">Issued from payment proof · bank reconciliation is separate.<br>Demonstration receipt · no actual payment</p></div>',action('close-modal','Close','btn')+action('print-receipt',icon('download')+' Print / save PDF','btn primary'));
}
function matchDialog(receiptId){
 const r=state.receipts.find(r=>r.id===receiptId),invoice=state.invoices.find(i=>i.id===r.invoiceId),analysis=analyzeStatement(state).receipts.find(row=>row.receiptId===receiptId);ui.matchReceiptId=receiptId;ui.selectedBank=ui.matchDraft?.receiptId===receiptId?ui.matchDraft.bankId:r.bankId||null;ui.matchCandidates=analysis?.candidateBankIds||[];
 Object.assign(collection('bank'),{page:1,query:''});
 modal('Review bank match',(analysis?.status==='ambiguous'||analysis?.status==='amount-mismatch'?'<div class="notice mb-16">'+esc(analysis.reason)+'</div>':'')+'<div class="two-columns"><section><p class="eyebrow">Centre receipt</p><h3 class="mt-16">'+r.id+' · '+studentById(r.studentId).name+'</h3><p class="report-total mt-8">'+money(r.amount)+'</p><dl class="detail-grid"><div><dt>Proof received</dt><dd>'+dateLabel(r.proofDate)+'</dd></div><div><dt>Receipt issued</dt><dd>'+dateLabel(r.issuedDate)+'</dd></div><div><dt>Payment reference</dt><dd>'+esc(invoice?.proofReference||'Not recorded')+'</dd></div><div><dt>Proof</dt><dd>'+action('view-proof','View payment proof','inline-link','data-id="'+r.invoiceId+'"')+'</dd></div></dl><div class="notice blue">The receipt issue date will stay unchanged.</div><div id="match-comparison" class="mt-16">'+matchComparison(r,ui.selectedBank)+'</div></section><section><p class="eyebrow mb-16">Bank statement entries</p>'+searchControl('bank','Search bank entries','Reference, amount or date')+'<div id="bank-results">'+bankResults()+'</div></section></div><div class="field mt-24"><label for="reconcile-note">Review note</label><input id="reconcile-note" value="'+esc(ui.matchDraft?.receiptId===receiptId?ui.matchDraft.note:r.note)+'" placeholder="Add a review note"></div>',action('close-modal','Cancel','btn')+action('confirm-match','Save bank match','btn primary'),true);
}
function matchComparison(r,bankId){
 ...
 const status=b.amount===r.amount?'Amounts match':'Amount difference',adjustment=b.date<r.issuedDate?'Date back':b.date>r.issuedDate?'Date forward':null;
 return '<div class="stack-sm">'+tag(status,status==='Amounts match'?'green':'red')+(adjustment?tag(adjustment,'blue'):'')+'<p class="small muted">HQ month: <strong>'+dateLabel(b.date,{day:undefined,month:'long',year:'numeric'})+'</strong></p>'+(b.amount!==r.amount?'<p class="small red-text">'+money(Math.abs(r.amount-b.amount))+' difference remains for follow-up.</p>':'')+'</div>';
}
...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
  ...
  const i=state.invoices.find(i=>i.id===id);modal('Invoice '+id,'<div class="receipt-paper"><div class="wordmark">Math<span>Concept</span></div><p class="small muted mt-16">'+centre.name+'</p><dl class="detail-grid"><div><dt>Student</dt><dd>'+studentById(i.studentId).name+'</dd></div><div><dt>Tuition period</dt><dd>'+i.period+'</dd></div><div><dt>Issued</dt><dd>'+dateLabel(i.issued)+'</dd></div><div><dt>Payment due</dt><dd>'+dateLabel(i.due)+'</dd></div></dl><p class="small">'+esc(i.description)+'</p><div class="receipt-total"><span>Total</span><span>'+money(i.amount)+'</span></div></div>',action('close-modal','Close','btn')+(!i.receiptId?action('submit-proof','Submit payment proof','btn primary','data-id="'+id+'"'):''));
 ...
 else if(a==='match-receipt'||a==='return-bank-review')matchDialog(id);
