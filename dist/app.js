...
function studentProfilePanel(id){
 ...
 const regular='<div class="student-regular-row"><dl class="student-facts">'+profileFact('Regular lesson',p.lessonDays.join(', ')+(p.lessonTime?' · '+p.lessonTime:''))+profileFact('Teacher',p.instructor)+profileFact('Course / duration',p.course+(p.lessonDuration?' · '+p.lessonDuration+' min':''))+profileFact('Payment reminders',p.paymentReminder?'Enabled':'Off')+(p.regularEffectiveDate?profileFact('Schedule effective from',profileDate(p.regularEffectiveDate)):'')+'</dl>'+action('change-regular-schedule','Change regular schedule','btn small','data-id="'+id+'"')+'</div>';
