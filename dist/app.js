...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
  ...
  const i=state.invoices.find(i=>i.id===id);modal('Invoice '+id,'<div class="receipt-paper"><div class="wordmark">Math<span>Concept</span></div><p class="small muted mt-16">'+centre.name+'</p><dl class="detail-grid"><div><dt>Student</dt><dd>'+studentById(i.studentId).name+'</dd></div><div><dt>Tuition period</dt><dd>'+i.period+'</dd></div><div><dt>Issued</dt><dd>'+dateLabel(i.issued)+'</dd></div><div><dt>Payment due</dt><dd>'+dateLabel(i.due)+'</dd></div></dl><p class="small">'+esc(i.description)+'</p><div class="receipt-total"><span>Total</span><span>'+money(i.amount)+'</span></div></div>',action('close-modal','Close','btn')+(i.proof?action('view-proof','Payment proof','btn','data-id="'+id+'"'):'')+(i.receiptId?action('view-receipt','Receipt','btn primary','data-id="'+i.receiptId+'"'):action('submit-proof',i.proof?'Replace proof':'Add payment proof','btn primary','data-id="'+id+'"')));
