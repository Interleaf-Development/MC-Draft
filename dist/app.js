...
function canDraw(){const a=currentAssignment();return !!a&&!ui.readonly&&!ui.showOriginal&&(ui.role==='teacher'&&a.status!=='prepared'||ui.role==='student'&&canStudentEditAssignment(a,ui.familyStudent));}
function worksheetPage(){
 ...
 const teacherActions=teacher?(a.status==='prepared'?action('release-prepared-work','Release to student','btn primary','data-id="'+a.id+'"'):action('return-corrections','Request corrections','btn','data-id="'+a.id+'"')+action('complete-work',icon('check')+' Mark complete','btn primary','data-id="'+a.id+'"')):'';
...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
  ...
  if(ui.role!=='teacher'||state.assignments.find(item=>item.id===id)?.status==='prepared')return;
