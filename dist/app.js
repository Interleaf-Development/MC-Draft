...
function receiptDialog(receiptId,revisionId=null){
 ...
 modal(t('Receipt','收據')+' '+(revisionId==='original'?r.id:revisionId||r.activeRevisionId||r.id),renderReceiptDocument(state,receiptId,{role:ui.role,revisionId}),action('close-modal',t('Close','關閉'),'btn')+(ui.role==='admin'&&i?.proof?action('view-proof','Payment proof','btn','data-id="'+esc(i.id)+'"'):'')+action('print-receipt',icon('download')+' '+t('Print / save PDF','列印／儲存 PDF'),'btn primary'));
