...
seedCentreVolume(state);seedTeacherSchedules(state);seedBusyAfternoons(state);normalizeParentLeave(state);normalizeStaffLeave(state);normalizeConversations(state);normalizeBillingAutomation(state);normalizeP6Progress(state);normalizeTwnSchedule(state);
...
const regularLabel = s => state.regularSchedules?.[s.id] ? (()=>{const rule=getRegularSchedule(state,s.id);return ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][rule.weekday-1]+' · '+time(rule.start);})() : s.id==='mia'&&state.assessment.enrolled ? (()=>{const b=state.bookings.find(b=>b.studentId==='mia'&&activeBooking(b));return b?dateLabel(b.date,{weekday:'long',day:undefined,month:undefined})+' · '+time(b.start):'Time to be confirmed';})() : s.regular;
...
function render() {
  ...
  const sidebarDemo = staffView ? '<div class="sidebar-demo">' + roleSwitcher + '<div class="sidebar-demo-actions">' + demoButtons + '</div><div class="sidebar-demo-date">30 September 2026</div></div>' : '';
...
function calendarCell(date,start,tutor,rowHeight){
 ...
 const offLabel=offTime?'Off time · '+tutorName(tutor)+' · '+dateLabel(date)+' · '+time(start)+'–'+time(start+60):'';
 ...
 return '<div class="calendar-cell'+(moving?' pick-target':'')+(offTime?' tutor-off':'')+(onLeave?' tutor-away':'')+'" style="--row-height:'+rowHeight+'px" data-slot data-date="'+date+'" data-start="'+start+'" data-tutor="'+tutor+'"'+(offTime?' title="'+esc(offLabel)+'"':'')+(moving?' tabindex="0" role="button" aria-label="Move lesson to '+esc(tutorName(tutor)+' · '+dateLabel(date)+' · '+time(targetStart)+(offTime?' · Off time':''))+'"':offTime?' role="group" aria-label="'+esc(offLabel)+'"':'')+'>'+bookings.map(b=>bookingChip(b,start)).join('')+(onLeave?'<span class="tutor-away-note">On leave</span>':'')+'</div>';
}
function timetable(dates,tutor,minimumHeights={}){
 ...
 return '<div class="timetable '+(ui.scheduleView==='day'?'day':'week')+'" style="--day-count:'+dates.length+'" role="group" aria-label="'+tutorName(tutor)+' schedule"><div class="timetable-corner">Time</div>'+days+rows+'<div class="timetable-end">'+time(hours.at(-1)+60)+'</div><div class="timetable-end-fill"></div></div>';
}
...
function schedulePage(){
 ...
 const offTimeLegend=dates.some(date=>SCHEDULE_HOURS.some(start=>isTutorOffTime(state,tutor,date,start,60)))?'<span><i class="legend-colour off"></i>Off time</span>':'';
 const calendar='<section class="panel" id="teacher-schedule" '+(admin?'role="tabpanel" aria-labelledby="teacher-tab-'+tutor+'"':'aria-label="My schedule"')+'><div class="calendar-toolbar"><div class="calendar-controls">'+action('prev-week',icon('left'),'icon-btn border','aria-label="Previous '+ui.scheduleView+'"')+action('next-week',icon('right'),'icon-btn border','aria-label="Next '+ui.scheduleView+'"')+'<span class="calendar-title">'+dateTitle+'</span>'+action('today','Today','btn small')+'</div><div class="calendar-tools"><div class="segmented">'+action('calendar-view','Day',ui.scheduleView==='day'?'active':'','data-view="day"')+action('calendar-view','Week',ui.scheduleView==='week'?'active':'','data-view="week"')+'</div>'+'</div></div><div class="calendar-scroll">'+timetable(dates,tutor)+'</div><div class="calendar-legend"><span><i class="legend-line"></i>Regular lesson</span><span><i class="legend-line red"></i>Rescheduled</span><span>'+icon('check','attendance-tick')+' Attended</span><span><i class="legend-colour green"></i>New student</span>'+offTimeLegend+'</div></section>';
...
function scheduleStudentInfo(){
 ...
 const booking=b.status==='moved'?'Moved to another lesson':b.status==='cancelled'?'Cancelled':b.status==='absent'?'Make-up pending':source?'Rescheduled from '+dateLabel(source.date):b.caseId?'Schedule adjustment make-up':'Regular lesson';
 ...
 const remainingDates=remaining.lessons.map(lesson=>{const date=new Date(lesson.date+'T12:00:00Z'),label=date.toLocaleDateString('en-US',{month:'short',day:'numeric',timeZone:'UTC'}),description=date.toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'short',year:'numeric',timeZone:'UTC'})+' · '+time(lesson.start)+'–'+time(lesson.start+lesson.duration);return '<span class="schedule-lesson-date" title="'+esc(description)+'" aria-label="'+esc(description)+'">'+esc(label)+'</span>';}).join('');
 const remainingInfo=s.source==='twn-schedule'&&!remaining.hasPaidPeriod?'':'<section class="schedule-remaining-lessons" aria-label="Remaining paid lessons"><h4>Lessons left'+(remaining.hasPaidPeriod?'<span aria-label="'+count+' scheduled lessons remaining">'+count+'</span>':'')+'</h4>'+(count?'<div class="schedule-lesson-dates">'+remainingDates+'</div>':'<p>'+(!remaining.hasPaidPeriod?'No paid lessons remaining.':'No dated lessons remaining.')+'</p>')+(remaining.pendingMinutes?'<p>+ '+remaining.pendingMinutes+' min to arrange</p>':'')+'</section>';
 return head+'<div class="schedule-student-identity"><div><h4>'+esc(s.name)+(s.level?' ('+esc(s.level)+')':'')+'</h4><p>'+esc(p.studentNumber)+(p.chineseName?' · '+esc(p.chineseName):'')+'</p></div></div>'+(s.source==='twn-schedule'&&!p.school&&!parent&&!p.parentMobile&&!p.parentPhone?'':'<dl class="schedule-student-facts">'+fact('School',p.school)+fact('Parent',parent+(p.parentRelation?' · '+p.parentRelation:''))+fact('Phone',p.parentMobile||p.parentPhone)+'</dl>')+(p.remark?'<div class="schedule-profile-note"><h4>Student / parent remarks</h4><p>'+esc(p.remark)+'</p></div>':'')+remainingInfo+'<details class="schedule-lesson-info"><summary>Selected lesson'+icon('down','sm')+'</summary><dl class="schedule-student-facts">'+fact('Date',dateLabel(b.date,{weekday:'short'}))+fact('Time',time(b.start)+'–'+time(b.start+b.duration))+fact('Teacher',tutorName(b.tutor))+fact('Attendance',attendance)+fact('Booking',booking)+'</dl></details>'+field('Lesson remark','<textarea id="schedule-booking-note" data-booking-id="'+esc(b.id)+'" rows="2" aria-label="Lesson remark" placeholder="Add a note for the teacher">'+esc(note)+'</textarea>')+'<div class="schedule-student-actions">'+action('save-schedule-remark','Save remark','btn small','id="schedule-save-remark" data-id="'+esc(b.id)+'"'+(note===(b.note||'')?' disabled':''))+'</div>'+action('change-regular-schedule','Change regular schedule','btn small w-full mt-16','data-id="'+esc(s.id)+'"')+'<div class="schedule-student-footer">'+action('student-profile','View profile','inline-link','data-id="'+esc(s.id)+'"')+'</div>'+'</div></section>';
}
...
function studentProfilePanel(id){
 ...
 const regular='<div class="student-regular-row"><dl class="student-facts">'+profileFact('Regular lesson',p.lessonSessions?.length?p.lessonSessions.map(slot=>slot.day+' · '+slot.time+' · '+slot.instructor).join('; '):p.lessonDays.join(', ')+(p.lessonTime?' · '+p.lessonTime:''))+profileFact('Teacher',p.instructor)+profileFact('Course / duration',p.course+(p.lessonDuration?' · '+p.lessonDuration+' min':''))+profileFact('Payment reminders',p.paymentReminder?'Enabled':'Off')+(p.regularEffectiveDate?profileFact('Schedule effective from',profileDate(p.regularEffectiveDate)):'')+(p.regularEndDate?profileFact('Schedule final date',profileDate(p.regularEndDate)):'')+'</dl>'+action('change-regular-schedule','Change regular schedule','btn small','data-id="'+id+'"')+'</div>';
 ...
 return '<section class="student-detail panel" aria-label="Student profile"><header class="student-profile-head"><div class="flex">'+avatar(s,'large')+'<div><h2>'+esc(p.englishName)+'</h2><div class="row-meta">'+([p.studentNumber,p.grade].filter(Boolean).length?esc([p.studentNumber,p.grade].filter(Boolean).join(' · '))+' · ':'')+(p.status==='paused'?'Paused':p.status==='assessment'?'Assessment':'Active')+(editing?' · Editing':'')+'</div></div></div><div class="student-profile-actions">'+(editing?action('cancel-profile-edit','Cancel','btn small','data-id="'+id+'"')+action('save-profile-edit','Save changes','btn primary small','data-id="'+id+'"'):action('edit-profile',draft?'Resume editing':'Edit details','btn small','data-id="'+id+'"')+action('view-student-folder',icon('folder','sm')+' Learning folder','btn small','data-id="'+id+'"'))+'</div></header>'+workbenchTabs+'<div class="student-profile-scroll" role="region" aria-label="'+esc(active)+'">'+content+'</div></section>';
}
function studentProfileHistory(id,p){
 ...
 if(tab==='Payment history'){
  ...
  return invoices.length?'<div class="table-scroll"><table><thead><tr><th>Invoice / period</th><th>Amount</th><th>Due</th><th>Payment / bank match</th><th></th></tr></thead><tbody>'+page.items.map(i=>{
   const receipt=state.receipts.find(r=>r.id===i.receiptId),match=receipt&&reconciliation(state,receipt);
   return '<tr><td>'+esc(i.id)+'<div class="row-meta">'+esc(i.period)+'</div></td><td>'+money(i.amount)+'</td><td>'+profileDate(i.due)+'</td><td>'+tag(receipt?paymentIssuedLabel(receipt):i.proof?'Proof received':'Awaiting proof',receipt?'green':i.proof?'blue':'')+(receipt?'<div class="row-meta">'+esc(receipt.id)+' · '+esc(match.status)+'</div>':'')+'</td><td><div class="flex wrap">'+action('view-invoice','Invoice','btn small','data-id="'+i.id+'"')+(receipt?action('view-receipt',paymentDocumentLabel(receipt),'btn small','data-id="'+receipt.id+'"'):'')+'</div></td></tr>';
...
function billingPage(){
 ...
 const shell=heading('Billing & reconciliation')+'<div class="billing-tabs-toolbar">'+tabs(['Payments','Invoices','HQ report'],ui.billingTab,'billing-tab')+'</div>';
 if(ui.billingTab==='Payments')return shell+bankCheckUI.render();
 if(ui.billingTab==='HQ report')return shell+directorReview();
 const labels={issued:'Acknowledgement issued',review:'Proof needs review',proof:'Needs screening',awaiting:'Awaiting payment',overdue:'Overdue'};
...
function scheduleQueues(){
 ...
 return '<section class="panel rail-card schedule-makeup-bin" aria-label="Pending make-ups"><div class="rail-title"><h3>Pending make-ups</h3>'+tag(pending.length)+'</div><div class="schedule-leave-dropzone" data-leave-dropzone role="region" aria-label="Leave bin">'+icon('inbox')+'<div><strong>Leave bin</strong><span>Drop lesson here · arrange later</span></div></div>'+(pending.length>5?searchControl('makeups','Search make-ups','Student or ID'):'')+(pending.length?makeupsPage.items.map(m=>{
...
function parentPayments(){
 ...
 return heading(t('Payments','繳費'),childSwitch())+'<div class="stack">'+(invoices.length?invoices.map(i=>{const status=invoiceStatus(i);return '<section class="panel"><div class="panel-head"><div><h3>'+esc(content(i.period))+'</h3><p class="small muted mt-8">'+i.id+' · '+t('Issued','發出日期')+' '+familyDate(i.issued,ui.role)+'</p></div>'+tag(i.receiptId?paymentIssuedLabel(i.receiptId):status==='review'?t('Proof needs attention','付款證明待覆核'):i.proof?t('Proof submitted','已提交付款證明'):t('Due','繳費限期')+' '+familyDate(i.due,ui.role),i.receiptId?'green':status==='review'?'amber':i.proof?'blue':'amber')+'</div><div class="panel-body between wrap"><div><h2>'+money(i.amount)+'</h2><p class="small muted mt-8">'+esc(content(i.description))+'</p></div><div class="flex">'+action('view-invoice',t('View invoice','查看繳費通知'),'btn','data-id="'+i.id+'"')+(i.receiptId?action('view-receipt',icon('receipt')+' '+paymentDocumentLabel(i.receiptId),'btn primary','data-id="'+i.receiptId+'"'):i.proof?action('view-proof',status==='review'?t('View result','查看結果'):t('View proof','查看付款證明'),'btn','data-id="'+i.id+'"'):action('submit-proof',icon('upload')+' '+t('Submit payment proof','提交付款證明'),'btn primary','data-id="'+i.id+'"'))+'</div></div></section>';}).join(''):'<div class="panel">'+empty(t('No invoices yet','暫時沒有繳費通知'),t('Your centre will issue an invoice after enrolment.','完成報名後，中心會發出繳費通知。'),14)+'</div>')+'</div>';
}
...
function worksheetPage(){
 ...
 const writingToolbar='<header class="student-writing-toolbar" aria-label="工作紙工具">'+action('back-work',icon('left'),'icon-btn','aria-label="返回工作簿"')+'<span class="student-writing-title">'+esc(w.code)+'</span><div class="student-writing-tools" role="group" aria-label="書寫工具">'+studentTools+'</div><details class="student-writing-options"><summary aria-label="更多工具">'+icon('more')+'</summary><div class="student-writing-popover">'+colourTools+'<label for="student-paper-zoom">紙張大小<select id="student-paper-zoom">'+[1,1.25,1.5,2].map(zoom=>'<option value="'+zoom+'"'+(zoom===(ui.paperZoom||1)?' selected':'')+'>'+Math.round(zoom*100)+'%</option>').join('')+'</select></label>'+original+'</div></details>'+action('toggle-work-notes',icon('message'),'icon-btn','aria-label="老師評語與筆記" aria-expanded="'+!!ui.workNotes+'" aria-controls="worksheet-notes"')+(editable?action('submit-work','交給老師','btn primary student-hand-in','data-id="'+a.id+'"'):'')+'</header>';
 const teacherActions=teacher?(a.status==='prepared'?action('release-prepared-work','Release to student','btn primary','data-id="'+a.id+'"'):w.catalogueOnly?'':action('return-corrections','Request corrections','btn','data-id="'+a.id+'"')+action('complete-work',icon('check')+' Mark complete','btn primary','data-id="'+a.id+'"')):'';
...
function matchDialog(receiptId,editing=false){
 ...
 const details='<dl class="detail-grid"><div><dt>Invoice</dt><dd>'+esc(r.invoiceId)+'</dd></div><div><dt>Issued</dt><dd>'+dateLabel(r.issuedDate)+'</dd></div><div><dt>Payment date</dt><dd>'+dateLabel(invoice?.claimedPaymentDate||r.proofDate||r.issuedDate)+'</dd></div><div><dt>Payer</dt><dd>'+esc(invoice?.proofReview?.extracted?.payer||invoice?.proofPayer||studentById(r.studentId).parent)+'</dd></div><div><dt>Reference</dt><dd>'+esc(invoice?.proofReference||'Not recorded')+'</dd></div></dl>';
 ...
 if(channel!=='non-face-to-face'){
  modal('Payment · '+esc(studentById(r.studentId).name),'<div class="billing-workspace" data-bank-review="'+esc(r.id)+'">'+payment+'<p class="small muted mt-16">'+(channel==='cash'?'Cash collection and deposit handling':'Cheque deposit and clearance')+' will be added after the workflow is agreed.</p></div>',action('close-modal','Close','btn'));
  return;
 }
 if(linked.status==='Matched'&&!editing){
  const bank=linked.bank;
  modal('Payment · '+esc(studentById(r.studentId).name),'<div class="billing-workspace" data-bank-review="'+esc(r.id)+'"><div class="two-columns">'+payment+'<section><p class="small muted">Bank deposit</p><h3 class="mt-8">'+money(bank.amount)+'</h3><dl class="detail-grid"><div><dt>Bank credited</dt><dd>'+dateLabel(bank.date)+'</dd></div><div><dt>HQ month</dt><dd>'+dateLabel(bank.date,{day:undefined,month:'long',year:'numeric'})+'</dd></div><div><dt>Payer</dt><dd>'+esc(bank.payer||invoice?.proofPayer||'Not recorded')+'</dd></div><div><dt>Reference</dt><dd>'+esc(bank.reference)+'</dd></div></dl>'+(linked.adjustment?'<p class="small muted">'+linked.adjustment+'</p>':'')+'</section></div>'+(r.note?'<p class="small">'+esc(r.note)+'</p>':'')+'</div>',action('change-bank-match','Change match','btn ghost','data-id="'+esc(r.id)+'"')+action('close-modal','Close','btn'),true);
  return;
 }
 const warning=analysis?.status==='ambiguous'?(ui.matchCandidates.length>1?ui.matchCandidates.length+' bank credits could match this payment.':'This bank credit could match more than one payment.'):analysis?.status==='amount-mismatch'?'The bank credit differs from the acknowledged amount.':analysis?.status==='missing'?'No matching bank credit found. Check the statement coverage and payment details.':'';
 modal('Bank match · '+esc(studentById(r.studentId).name),'<div class="billing-workspace" data-bank-review="'+esc(r.id)+'">'+(warning?'<p class="billing-status review">'+esc(warning)+'</p>':'')+'<div class="two-columns"><div>'+payment+'<div id="match-comparison" class="mt-16">'+matchComparison(r,ui.selectedBank)+'</div></div><section>'+searchControl('bank','Search bank deposits','Payer, reference or date')+'<div id="bank-results">'+bankResults()+'</div></section></div>'+field('Review note','<input id="reconcile-note" value="'+esc(ui.matchDraft?.receiptId===receiptId?ui.matchDraft.note:r.note||'')+'" placeholder="Optional">')+'</div>',action('close-modal','Cancel','btn')+action('confirm-match','Save bank match','btn primary'),true);
}
...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
 ...
 else if(a==='toggle-work-notes'){ui.workNotes=!ui.workNotes;document.body.classList.toggle('work-notes-open',ui.workNotes);$('.student-writing-toolbar [data-action=toggle-work-notes], .drawing-tools [data-action=toggle-work-notes]')?.setAttribute('aria-expanded',String(ui.workNotes));$(ui.workNotes?'.notes-heading .icon-btn':'.student-writing-toolbar [data-action=toggle-work-notes], .drawing-tools [data-action=toggle-work-notes]')?.focus();}
 ...
 else if(a==='report-exceptions'){ui.billingTab='Payments';bankCheckUI.reset();render();bankCheckUI.openReviewQueue();}
  ...
  if(change(()=>{if(!ui.selectedBank)throw new Error('Select a bank transaction.');const result=matchReceipt(state,ui.matchReceiptId,ui.selectedBank);state.receipts.find(r=>r.id===ui.matchReceiptId).note=note;},'Bank match saved. Parent acknowledgement unchanged.')){ui.matchDraft=null;closeModal();}
