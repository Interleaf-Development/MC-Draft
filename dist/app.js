...
function schedulePage(){
 ...
 return heading('Schedule')+teacherTabs+(moving?'<div class="move-banner"><span>Choose a new time for <strong>'+studentById(moving.studentId).name+'</strong>.</span>'+action('cancel-move','Cancel','btn ghost small')+'</div>':'')+'<div class="schedule-layout"><section class="panel" id="teacher-schedule" role="tabpanel" aria-labelledby="teacher-tab-'+ui.scheduleTutor+'"><div class="calendar-toolbar"><div class="calendar-controls">'+action('prev-week',icon('left'),'icon-btn border','aria-label="Previous '+ui.scheduleView+'"')+action('next-week',icon('right'),'icon-btn border','aria-label="Next '+ui.scheduleView+'"')+'<span class="calendar-title">'+dateTitle+'</span>'+action('today','Today','btn small')+'</div><div class="calendar-tools"><div class="segmented">'+action('calendar-view','Day',ui.scheduleView==='day'?'active':'','data-view="day"')+action('calendar-view','Week',ui.scheduleView==='week'?'active':'','data-view="week"')+'</div>'+action('find-schedule-student',icon('search')+' Find student','btn small')+action('new-booking',icon('plus')+' Add lesson','btn primary small')+'</div></div><div class="calendar-scroll">'+timetable(dates,ui.scheduleTutor)+'</div><div class="calendar-legend"><span><i class="legend-line"></i>Regular lesson</span><span><i class="legend-line red"></i>Rescheduled</span><span><i class="legend-line gray"></i>Original booking</span><span>'+icon('check','attendance-tick')+' Attended</span></div></section><aside class="schedule-rail stack">'+scheduleQueues()+'</aside></div>';
}
...
function studentProfilePanel(id){
 ...
 const workbenchTabs='<div class="student-history-tabs tabs" aria-label="Student workbench">'+['Student information','Student schedule','Payment history','E-coupon & referral','Marketing','SMS history'].map(item=>action('profile-history-tab',item,active===item?'active':'','data-value="'+esc(item)+'" aria-pressed="'+(active===item)+'"')).join('')+'</div>';
...
function handleAction(a,id,button){
 ...
 if(a==='new-booking'){
  ...
  const i=state.invoices.find(i=>i.id===id);modal('Invoice '+id,'<div class="receipt-paper"><div class="wordmark">Math<span>Concept</span></div><p class="small muted mt-16">'+centre.name+'</p><dl class="detail-grid"><div><dt>Student</dt><dd>'+studentById(i.studentId).name+'</dd></div><div><dt>Tuition period</dt><dd>'+i.period+'</dd></div><div><dt>Issued</dt><dd>'+dateLabel(i.issued)+'</dd></div><div><dt>Payment due</dt><dd>'+dateLabel(i.due)+'</dd></div></dl><p class="small">'+esc(i.description)+'</p><div class="receipt-total"><span>Total</span><span>'+money(i.amount)+'</span></div></div>',action('close-modal','Close','btn')+(!i.proof?action('submit-proof','Submit payment proof','btn primary','data-id="'+id+'"'):''));
