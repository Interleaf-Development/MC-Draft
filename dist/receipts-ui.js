...
const dateText = (date, options = {}) => new Date(date + 'T00:00:00Z').toLocaleDateString('zh-HK', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC', ...options });
...
const bankLabels = { matched: '已核對銀行入賬', ready: '待核對銀行入賬', ambiguous: '入賬配對不明確', 'amount-mismatch': '金額不符', missing: '未找到入賬' };
const statusFilters = { all: '所有收據', review: '待覆核', ready: '待核對銀行入賬', matched: '已核對銀行入賬' };
const bankTone = status => status === 'matched' ? 'matched' : status === 'ready' ? 'waiting' : 'review';
const reviewPriority = row => bankTone(row.bankStatus) === 'review' ? 0 : 1;
const bankSwatch = tone => `<span class="receipts-bank-swatch is-${tone}" aria-hidden="true"></span>`;
...
export function receiptRegister(state, { date = TODAY, query = '', status = 'all' } = {}) {
  ...
  const rows = (state.receipts || []).filter(receipt => validDate(receipt.issuedDate) && receipt.issuedDate >= period.start && receipt.issuedDate <= period.end).map(receipt => {
    ...
    const student = studentIndex.get(receipt.studentId) || { id: receipt.studentId, name: receipt.studentId || '未能識別學生', number: '', parent: '' };
...
export function createReceiptsUI({ getState, render, openReceipt, openProof, openReview }) {
  ...
  function workspace() {
    ...
    const columns = result.groups.map(group => `<section class="receipts-day-column" aria-label="${dateText(group.date, { weekday: 'long' })}"><h3 class="receipts-day-heading${group.date === TODAY ? ' is-today' : ''}"><span>${dateText(group.date, { weekday: 'short', day: undefined, month: undefined, year: undefined })}</span><time datetime="${group.date}">${dateText(group.date, { year: undefined })}</time></h3><div class="receipts-day-list">${group.rows.length ? group.rows.map(({ receipt, student, bankStatus }) => button('open', `<span class="receipts-entry-name">${esc(student.name)}</span><span class="receipts-entry-amount">${money(receipt.amount)}</span>`, `data-id="${esc(receipt.id)}" title="${bankLabels[bankStatus]}" aria-label="查看 ${esc(student.name)} 的收據 ${esc(receipt.id)}，${money(receipt.amount)}，${dateText(group.date)} 發出，${bankLabels[bankStatus]}"`, `receipts-entry is-${bankTone(bankStatus)}`)).join('') : '<span class="receipts-day-empty" aria-label="尚未發出收據">—</span>'}</div></section>`).join('');
    return `<div class="billing-workspace receipts-workspace"><div class="receipts-toolbar"><div class="receipts-period-controls">${button('previous', arrow('left'), 'aria-label="上一週"', 'icon-btn border')}${button('next', arrow('right'), 'aria-label="下一週"', 'icon-btn border')}<strong class="receipts-period-title">${esc(periodTitle)}</strong>${button('today', '本週')}</div><div class="receipts-date-controls"><input id="receipts-search" type="search" value="${esc(query)}" placeholder="搜尋收據" aria-label="按學生、家長、收據或付款參考編號搜尋收據"><select id="receipts-status" aria-label="收據銀行對數狀態">${Object.entries(statusFilters).map(([value, label]) => `<option value="${value}"${status === value ? ' selected' : ''}>${label}</option>`).join('')}</select><input id="receipts-date" type="date" value="${date}" aria-label="選擇收據週次"></div></div>${(query || status !== 'all') && !result.total ? '<p class="small muted receipts-empty-search" role="status">本週沒有符合條件的收據。</p>' : ''}<section class="panel receipts-board" aria-label="按發出日期排列收據" tabindex="0"><div class="receipts-week-grid">${columns}</div></section><div class="receipts-bank-legend" aria-label="銀行對數狀態圖例"><span>${bankSwatch('review')}待覆核</span><span>${bankSwatch('matched')}已核對銀行入賬</span><span>${bankSwatch('waiting')}待核對銀行入賬</span></div></div>`;
  }
