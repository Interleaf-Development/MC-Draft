...
export function createRegularScheduleUI({getState,getViewer,change,modal,closeModal,openReceipt,getRoot=()=>document}) {
 ...
 function showReview(){
  ...
  const choice=delta>0?'<fieldset class="regular-decision"><legend>增加 '+delta+' 堂</legend><label class="check-option"><input type="radio" name="regular-decision" value="allow"><span><strong>批准額外課堂，維持學費，安排 '+preview.proposedCount+' 堂</strong><small>修訂收據以包括新增的課堂。</small></span></label><label class="check-option"><input type="radio" name="regular-decision" value="decline"><span><strong>不批准額外課堂，維持 '+preview.beforeCount+' 堂</strong><small>'+omitted.map(l=>dateText(l.date)).join('、')+' 不設課堂。</small></span></label></fieldset>':delta<0?'<fieldset class="regular-decision"><legend>減少 '+Math.abs(delta)+' 堂</legend><label class="check-option"><input type="radio" name="regular-decision" value="credit"><span><strong>提供 '+Math.abs(delta)+' 堂補堂</strong><small>已安排 '+preview.proposedCount+' 堂，另有 '+Math.abs(delta)+' 堂由客服安排。</small></span></label><label class="check-option"><input type="radio" name="regular-decision" value="decline"><span><strong>不提供補堂，維持 '+preview.proposedCount+' 堂</strong><small>不另補堂或退款；修訂收據列出減少後的課堂，學費維持不變。</small></span></label></fieldset>':'<p class="small muted">課堂數目維持不變，修訂收據將列出新的上課日期。</p>';
