...
Regenerate the downloadable Chinese PDF after proposal wording changes with `python3 scripts/build-proposal-pdf.py` (Python: `reportlab`, `lxml`, `svglib`, `pypdf`, `Pillow`; Node.js available on PATH or passed with `--node`). The builder reads the current proposal content, includes both student options, and uses the six demo screenshots in `dist/proposal/assets/pdf/`. It writes the PDF and its source manifest under `dist/proposal/`, plus a local copy under `output/pdf/`. Refresh screenshots when the demonstrated screens change. Tests reject a PDF whose content sources or screenshot inputs have changed since export.
