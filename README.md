...
The interactive project proposal is at **[/proposal/](https://mc-draft-rho.vercel.app/proposal/)** on the same Vercel site. Edit `dist/proposal/` for future proposal changes; it is a standalone document with its own scripts and styles. It contains six chapters (overview, student experience, teaching materials, teacher and home learning workflows, centre/HQ management and parent app) in a continuous document, with [Hong Kong Traditional Chinese](https://mc-draft-rho.vercel.app/proposal/?lang=zh-HK) shown by default and English accessible through `?lang=eng`. The topbar identifies it as MathConcept Proposal V1 and offers a PDF download; the PDF places the two student options in order and uses screenshots for the interactive demos. Proposal translations live in `content.zh-HK.js` and `locale.js`; the embedded application keeps its existing staff/family languages. Section 2 (學生體驗) compares paper with smartpen capture (option 1, selected by default) and tablet writing (option 2, `?solution=2`) at `#learning`. Section 3 (教材, `#materials`) contains material management and the cross-centre school knowledge base. Section 4 (老師及課後流程, `#teaching`) contains teacher assignment/marking, AI assistance and home practice. The teacher and game demos remain outside the student option switch. Teaching, student, parent, scheduling, billing and branch examples embed the actual shared application inline. Only one application frame is active at a time. `?proposal=1` stores its data in a separate sessionStorage namespace, shared across these chapter views in this tab, and never reads or changes the ordinary demo's localStorage. Native authoring, migration and controlled printing are described as proposed features rather than simulated application screens.

Regenerate the downloadable Chinese PDF after proposal wording or screenshot changes with `python3 scripts/build-proposal-pdf.py` (Python: `reportlab`, `lxml`, `svglib`, `pypdf`, `Pillow`; Node.js available on PATH or passed with `--node`). The builder reads the current proposal content, includes both student options, and uses the demo screenshots in `dist/proposal/assets/pdf/`. Compact device galleries show the student's binder, worksheet and stamps, and the parent's home, handbook, payments and messages; the teacher, game, scheduling and billing demos each retain a single screenshot. It writes the PDF and its source manifest under `dist/proposal/`, plus a local copy under `output/pdf/`. Refresh screenshots when the demonstrated screens change, then inspect the rendered PDF. Tests reject a PDF whose content sources or screenshot inputs have changed since export.
...
3. Payment proof requires staff approval before a receipt is issued. Simulated proof checks assist staff but do not automatically approve payment. Original proof acknowledgements remain historical documents.
...
Recurring tuition is **HK$2,000 for a nominal eight-lesson package valid for two calendar months**. Each student has their own period, such as September–October or October–November. The usual seven, eight or nine weekly lessons in a period do not change the fee or trigger a refund; the teaching schedule averages 48 weeks annually. The next invoice is issued on the **20th of the preceding month** and due on the **following month’s 20th**: October–November is invoiced on 20 September and due on 20 October. First-enrolment payment approval activates enrolment; renewal approval does not repeat activation.
...
- The receipt receives a new document version such as **R-1028-A1**, with exact lesson dates and any make-up entitlement. It retains the original payment date, receipt date and paid amount, and shows the actual amendment date and reason. Original and previous versions remain available under **Receipt versions**. The bank link and single payment ledger entry are preserved.
...
- Open **`/game1/`** for the standalone Hong Kong Chinese kart game. It is embedded in the proposal’s teaching materials chapter as an interactive practice example.
...
- The blue rival answers exactly **7/8** correctly and the green rival **4/8**, with their mistake order shuffled on every race. They choose answer lanes and receive the same boosts, spins and recovery as the player. Positions follow actual distance, so opponents can be passed. Small starting leads settle tied scores: **8/8 gives first place, 5–7/8 second, 0–4/8 third**.
